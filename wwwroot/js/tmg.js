window.onload = async function () {
    ...
    if (chartData) {
        const jChartData = JSON.parse(chartData)
        const defect = jChartData.findIndex(item => item[5] > 300000)
        if (defect !== -1) {
            jChartData.splice(defect)
            localStorage.setItem("tmg_pool_chart_data2", JSON.stringify(jChartData))
        }
    }

    //Update Modal behaviour with bootstrap
...
const Global = {
    server: 'https://wallet.signum-chain.com',
    wallet: undefined,
...
function showError(message) {
    ...
    const myModalElement = document.getElementById('modal_window');
   ...
   // document.getElementById("modal_window").classList.add('open show');
    myModal.show();
}
